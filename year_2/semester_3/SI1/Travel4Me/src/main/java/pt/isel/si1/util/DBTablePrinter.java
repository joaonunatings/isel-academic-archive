...
public class DBTablePrinter {
    ...
    public static void printTable(Connection conn, String tableName, int maxRows, int maxStringColWidth) {
        if (conn == null) {
            System.err.println("util.DBTablePrinter Error: No connection to database (Connection is null)!");
            return;
        }
        if (tableName == null) {
            System.err.println("util.DBTablePrinter Error: No table name (tableName is null)!");
            return;
        }
        if (tableName.length() == 0) {
            System.err.println("util.DBTablePrinter Error: Empty table name!");
            return;
        }
        if (maxRows < 1) {
            System.err.println("util.DBTablePrinter Info: Invalid max. rows number. Using default!");
        ...
        try {
            if (conn.isClosed()) {
                System.err.println("util.DBTablePrinter Error: Connection is closed!");
                return;
            }
            ...
            System.err.println("SQL exception in util.DBTablePrinter. Message:");
    ...
    public static void printResultSet(ResultSet rs, int maxStringColWidth) {
        try {
            if (rs == null) {
                System.err.println("util.DBTablePrinter Error: Result set is null!");
                return;
            }
            if (rs.isClosed()) {
                System.err.println("util.DBTablePrinter Error: Result Set is closed!");
                return;
            }
            if (maxStringColWidth < 1) {
                System.err.println("util.DBTablePrinter Info: Invalid max. varchar column width. Using default!");
