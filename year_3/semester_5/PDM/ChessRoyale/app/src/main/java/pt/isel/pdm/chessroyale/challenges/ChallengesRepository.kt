package pt.isel.pdm.chessroyale.challenges
...
/**
 * The repository for the ChessRoyale challenges, implemented using Firebase.
 */
class ChallengesRepository {
    ...
    fun withdrawChallenge(challengeId: String, onComplete: (Result<Unit>) -> Unit) {
        ...
        doc.get().addOnCompleteListener {
            if (it.result?.exists() == true) {
                doc.delete().addOnSuccessListener { onComplete(Result.success(Unit)) }
            } else {
                onComplete(Result.failure(Throwable("Document doesnt exist!")))
            }
        }
    }
    ...
    // Unsubscribes from the challenge
