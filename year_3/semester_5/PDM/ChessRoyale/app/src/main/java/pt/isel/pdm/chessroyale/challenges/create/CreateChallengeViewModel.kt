...
class CreateChallengeViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun createChallenge(name: String, message: String) {
        getApplication<ChessRoyaleApplication>().challengesRepository.publishChallenge(
            name = name,
            message = message,
            onComplete = {
                _created.value = it
    ...
    fun removeChallenge() {
        ...
        val repo = getApplication<ChessRoyaleApplication>().challengesRepository
    ...
    private fun waitForAcceptance(challengeInfo: ChallengeInfo) {
        subscription =
            getApplication<ChessRoyaleApplication>().challengesRepository.subscribeToChallengeAcceptance(
